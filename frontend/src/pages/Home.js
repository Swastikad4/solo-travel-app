    ...
    <div>
      ...
      <section className="hero">
        ...
        <div className="hero-content">
          <h1>
            Explore your<br />
            <span className="accent-text">place to travel</span>
          </h1>
          <p className="hero-subtitle">
            Discover stunning destinations, connect with fellow solo travelers,
          ...
          <div className="search-suggestions">
            {["Paris", "Tokyo", "Bali", "London", "Bangkok", "Iceland"].map(
              (s) => (
                <button
                  key={s}
                  className="suggestion-chip"
                  onClick={() => navigate(`/destination/${s.toLowerCase()}`)}
                >
                  {s}
                </button>
              )
      ...
      <section className="section" id="destinations">
        <div className="section-header">
          ...
          <p className="section-subtitle">
            Hand-picked destinations perfect for solo explorers, rated by
            safety, budget, and overall solo experience.
          </p>
        </div>
            ...
            {destinations.map((dest) => (
              ...
              >
                <div className="dest-card-img-wrapper">
                  ...
                  />
                  {dest.soloScore && (
                    <div className="dest-card-solo-score">
                      ⭐ {dest.soloScore} Solo Score
                    </div>
                  )}
                </div>
                <div className="dest-card-body">
                  ...
                  <div className="dest-card-meta">
                    {dest.avgBudget && (
                      <span className="dest-card-meta-item">
                        <span className="icon">💰</span> {dest.avgBudget}
                      </span>
                    )}
                    {dest.bestTime && (
                      <span className="dest-card-meta-item">
                        <span className="icon">📅</span> {dest.bestTime}
                      </span>
                    )}
                    {dest.safetyRating && (
                      <span className="dest-card-meta-item">
                        <span className="icon">🛡️</span> {dest.safetyRating}/5
                      </span>
                    )}
                  </div>
                </div>
